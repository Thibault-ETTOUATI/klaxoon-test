...
const BookmarksPage: FunctionComponent = () => {
    ...
    const isAuthorizedUrl = (url: string): boolean => {
        return url.includes(GLOBAL.AUTHORIZED_WEBSITE.FLICKR.slice(0, GLOBAL.AUTHORIZED_WEBSITE.FLICKR.length - 1))
            || url.includes(GLOBAL.AUTHORIZED_WEBSITE.VIMEO.slice(0, GLOBAL.AUTHORIZED_WEBSITE.VIMEO.length - 1));
    };

    // Sort the bookmark list from newest to oldest
    const sortArrayFromNewestToOldest = (list: Bookmark[]) => {
        list.sort((previous: Bookmark, next: Bookmark) => (previous.id < next.id) ? 1 : -1);
    };

    // Update bookmarks list by adding a new one
    const onAddBookmark = (url: string): void => {
        getBookmark(url).then((res: Bookmark) => {
            if(!res.error && isAuthorizedUrl(res.url)) {
                const newBookmarksList = [...bookmarksList];
                const todayDate = Date.now();
                newBookmarksList.push({...res, upload_date_on_app: todayDate, id: todayDate});
    ...
    const onRemoveBookmark = (id: number) => {
        setBookmarksList(bookmarksList.filter((bookmark: Bookmark) => bookmark.id !== id));
    };
    ...
    return (
        <>
            ...
            <BookmasksList bookmarksList={bookmarksList} onRemoveBookmark={onRemoveBookmark}/>
        </>
    );
};
