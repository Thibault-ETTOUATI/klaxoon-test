...
export default class TimeUtils {
    ...
    static getNumberOfMinutes(seconds: number): number {
        return Math.floor((seconds - (TimeUtils.getNumberOfHours(seconds) * 3600)) / 60);
    };

    // Calculate number of seconds
    static getNumberOfSeconds(seconds: number): number {
        return seconds - (TimeUtils.getNumberOfHours(seconds) * 3600) - (TimeUtils.getNumberOfMinutes(seconds) * 60);
    };
